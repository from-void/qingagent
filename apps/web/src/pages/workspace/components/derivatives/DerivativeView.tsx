...
export function DerivativeView(props: {
  ...
  const toolbar = <div className="ws-deriv-toolbar">
    <div className="ws-deriv-actions">
      {!generationUnconfirmed ? <div className="ws-deriv-regen-anchor">
        ...
        <button className="ws-docfn-btn" title={derivativeMutationEnabled ? "重新生成" : "当前青简引擎暂不支持重新生成"} aria-label="重新生成" disabled={!derivativeMutationEnabled} onClick={() => setModalOpen(true)}><RegenIcon/></button>
      </div> : null}
      ...
      <div className="ws-export-anchor" ref={moreRef}><button className="ws-docfn-btn" title="更多操作" aria-label="更多操作" onClick={() => { setExportOpen(false); setMoreOpen((value) => !value); }}><MoreIcon/></button>
        {moreOpen ? <div className="ws-export-menu" role="menu"><button className="ws-export-item is-danger" disabled={!derivativeMutationEnabled} title={derivativeMutationEnabled ? undefined : "当前青简引擎暂不支持删除稿件"} onClick={() => void deleteDraft()}>删除稿件</button></div> : null}
      </div>
    </div>
  </div>;
  ...
  return <section ref={viewRef} className={`ws-deriv-view ws-deriv-${descriptor.dtype}${mode === "phone" ? " is-phone" : " is-desktop"}`}>
    ...
    {isTranslation && !pmDoc ? <div className="ws-deriv-empty"><strong>{generationUnconfirmed ? "暂未确认翻译结果" : "该语言还没有译文"}</strong>{generationUnconfirmed ? <span>请稍后刷新查看，确认结果后再决定是否重新生成</span> : null}<div className="ws-deriv-empty-actions"><button className="ws-deriv-primary" disabled={!generationUnconfirmed && !derivativeMutationEnabled} title={!generationUnconfirmed && !derivativeMutationEnabled ? "当前青简引擎暂不支持生成译文" : undefined} onClick={generationUnconfirmed ? () => { void props.onRefresh(); } : () => setModalOpen(true)}>{generationUnconfirmed ? "刷新查看" : "生成该语言"}</button></div></div>
