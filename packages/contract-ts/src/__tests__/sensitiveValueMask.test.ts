...
describe("敏感值脱敏优先级", () => {
  ...
  it.each([
    ...
    "金额 1380013800 元",
    "快递单号 SF1234567890",
    // 4532015112830366 可过 Luhn；物流标签仍必须优先保留为业务编号。
    "快递单号 4532015112830366",
    "年份 2026",
  ])("正常业务数字继续原样保留：%s", (input) => {
    expect(maskPersistedReviewIgnoreValue(input)).toBe(input);
  });

  it("同一组可过 Luhn 的数字仅银行卡语境打码，快递单号不误伤", () => {
    expect(maskPersistedReviewIgnoreValue("快递单号 4532015112830366；银行卡 4532015112830366"))
      .toBe("快递单号 4532015112830366；银行卡 4532********0366");
  });
