...
安全提示：为保留登录和下载体验，交互式浏览器不会固定网站解析到的 IP，校验后仍存在 DNS 变化窗口。请仅在可信的本机网络环境中使用，不要让 Agent 自动浏览不可信站点；可配置首站域名白名单进一步缩小风险。
...
- 页面同时提供多种登录方式且用户尚未指定时，先单独用 `askUserQuestion` 让用户选择登录方式；它只用于选择，不能索取任意文本凭据。
- 手机号 + 短信验证码：登录页选「验证码登录 / 手机号登录」→ 在聊天内索取手机号
  → `browser_type` 填入 → `browser_click`「获取验证码 / 发送验证码」→ 在聊天内索取刚收到的短信验证码
  → `browser_type` 填入验证码 → `browser_click` 登录/提交 → `browser_snapshot` 确认已登录。
- 账号密码登录：在聊天内索取账号与密码 → `browser_type` 分别填入 → 提交。
- 浏览器会话在等待用户补充凭据期间保持存活，拿到验证码后继续在原页面操作即可。
