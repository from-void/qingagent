...
- `clusterBkg` / `clusterBorder` 是所有分区的默认色；要区分不同分区，给 `subgraph` 稳定 ASCII id 后写 `style 分区id fill:#浅色,stroke:#深色`，并只使用画布可保真的 `fill`、`stroke`、`color`、`stroke-width`；`stroke-dasharray`、节点几何或字号属性会让可视化编辑器降级为 Mermaid 预览。
