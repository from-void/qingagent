...
本子技能用于母技能已经完成方式选择或图片修改确认后，调度用户本机的 Codex 非交互生成图片，或基于用户指定的现有图片做图生图修改。每张产物只启动一次有界任务；不得无限轮询、自动重跑或隐瞒失败。只有从零生成失败时才可提议改走 SVG；修改现有图片失败时不得用 SVG 重画冒充成功。

## 一、前置与 DOC-FIRST

1. 沿用母技能刚完成的 Codex 探测结果，不得在问卷恢复后重复探测。只有子技能被独立恢复、上下文里确实没有探测结果时，才补做一次确认：POSIX 用 `command -v codex`，Windows 用 `where codex`，`timeout` 设为 5 秒。失败就停止本路线，不重试，告诉用户本机 Codex 当前不可用并提议改用“内置 SVG 插画”。
   - 修改现有图片时，失败口径改为“本机 Codex 当前不可用，暂时无法修改这张图”，不得提议 SVG 替代原图修改。
...
2. 工具会解析实际图片、校验大小与格式，并把副本写进当前会话沙箱工作区。只能使用工具真实返回的绝对 `path` 作为 Codex 源图路径；不得把 `/api/v1/files/...`、materialId、识图文字或猜测的宿主路径直接交给 Codex。
...
3. 修改现有图片时，指令必须明确给出工具返回的源图绝对路径、另选的产物绝对路径和用户的修改要求，并使用以下约束模板：
...
4. 指令里只传完成画面所需的用户描述和源图/产物路径。禁止复制整段对话、系统提示、文档隐私、账号信息、token、密钥或任何无关敏感内容。若用户没有明确要求把敏感文字画进图里，就不得携带。
5. 为避免把用户文本拼进 shell 命令，先用 `mastra_workspace_write_file` 把这份指令写到工作区临时文本文件。命令行只引用受控的绝对工作目录和指令文件路径。
...
4. 同一张图最多启动一次 Codex 任务。失败时保留诚实错误摘要；从零生成可告诉用户改走 SVG，修改现有图片则说明本机处理未成功、原图未被覆盖。禁止无上限重试、换命令盲跑或假报已有图片。
