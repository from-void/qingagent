...
6. 从零生成或修改位图时，为避免把用户文本拼进 shell 命令，优先用 `mastra_workspace_write_file` 把这份指令写到 `/workspace/codex-image-prompt-<短标识>.txt` 这类工作区绝对路径。不得用 `echo`、shell 重定向或 PowerShell 命令拼写指令文件。命令行只引用受控的绝对工作目录和指令文件路径。本步骤不得用于 SVG 修改。
