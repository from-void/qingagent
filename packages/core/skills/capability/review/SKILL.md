...
1. **单独审查当前文档**（包括菜单 query）：纯批注模式，不改稿。先 `readDraft` 读取当前全文，确定问题统一用 `create_annotation_groups` 呈现，由用户逐条处理。敏感词词库中的 `replacementHint` 也只是供上下文判断的候选，不构成直接改稿授权。
...
2. 单独审查只把确定问题交给 `create_annotation_groups`；没有确定问题时不调用，并且只回复「本轮审查没有发现需要处理的问题」，不得静默收尾。敏感词全部命中都必须呈现，拿不准时也不得丢弃，但没有保持语义、语法与文体的安全改写时必须省略 `suggestion`。
