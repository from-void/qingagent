...
vi.mock("@qingagent/db", () => ({
  ...
  listReviewDismissalSignals: vi.fn(async () => []),
...
describe("create_annotation_groups 来源引句校验", () => {
  ...
  it("记住后重跑过滤同命中文本与规则，不误伤不同位置文本或不同规则", async () => {
    const { state, tool } = setup();
    state.doc!.content.push({
      type: "paragraph",
      attrs: { blockId: "p-2" },
      content: [{ type: "text", text: "另一处行动建议仍然空泛。" }],
    });
  ...
  it("预置隐私审查用打码后的命中文本匹配记忆信号", async () => {
    const { state, tool } = setup();
    state.doc!.content = [{
      type: "paragraph",
      attrs: { blockId: "p-contact" },
      content: [{ type: "text", text: "联系电话 13912345678。" }],
    }];
    vi.mocked(listReviewDismissalSignals).mockResolvedValueOnce([{
      id: "dismissal-privacy",
      docId: state.docId,
      origin: "privacy",
      summary: "手机号 139****5678",
      quote: "139****5678",
    ...
    expect(state.annotationGroups).toEqual([]);
    expect(replaceAnnotationGroupsByOrigin).not.toHaveBeenCalled();
  });
