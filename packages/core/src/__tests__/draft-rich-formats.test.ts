...
describe("draft rich formats AI 草稿工具链路", () => {
    ...
    const failed = await rollbackEditDraft.execute!({
      ops: [
        ...
        },
        {
          ...
          position: "after",
          ref: "block-tasks",
          blocks: [{ type: "callout", tone: "info", runs: [{ text: "也不会落盘" }] }],
        },
  ...
  it("AI-IR 脏输入:taskList 裸 run[][] 被修复,math+bold 混用不崩,非法 block 给可读块级错误", async () => {
    ...
    const repaired = await editDraft.execute!({
      ops: [{
        ...
        blocks: [{
          ...
          items: [
            [{ text: "裸数组任务" }],
            [{ text: "裸数组公式 ", marks: [{ type: "bold" }] }, { text: "a^2+b^2", marks: [{ type: "math" }] }],
          ],
        }],
      }],
    }, ctx) as any;
    ...
    const emptyMath = await editDraft.execute!({
      ops: [{
        ...
        position: "after",
        ref: "block-anchor",
        blocks: [{ type: "blockMath", latex: "" }],
      }],
    }, ctx) as any;
    expect(emptyMath.ok).toBe(false);
    expect(emptyMath.failedOpIndex).toBe(0);
    expect(emptyMath.error).toContain("block 0");
    ...
    const invalidCallout = await editDraft.execute!({
      ops: [{
        ...
        position: "after",
        ref: "block-anchor",
        blocks: [{ type: "callout", tone: "urgent", runs: [{ text: "非法 tone" }] }],
      }],
    }, ctx) as any;
    expect(invalidCallout.ok).toBe(false);
    expect(invalidCallout.failedOpIndex).toBe(0);
    expect(invalidCallout.error).toContain("block 0");
    expect(invalidCallout.error).toContain("tone");
