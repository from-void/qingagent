...
describe("startToolHeartbeat", () => {
  ...
  it("按 interval 周期 emit,seq 从 1 递增、type 恒为 tool-heartbeat、带 tool 名", () => {
    const write = vi.fn();
    ...
    vi.advanceTimersByTime(5_000 * 3); // 推进 3 个周期
  ...
  it("默认 intervalMs 为 10s:推进 9.9s 不 emit,满 10s emit 一次", () => {
    const write = vi.fn();
    const stop = startToolHeartbeat({ writer: { write } }, { tool: "t" });

    vi.advanceTimersByTime(9_999);
    expect(write).not.toHaveBeenCalled();
    vi.advanceTimersByTime(1); // 凑满 10_000
  ...
  describe("intervalMs 夹取到 [1000,30000]", () => {
    ...
    it("过小(100)被夹到下界 1000:1000ms 才 emit", () => {
      const write = vi.fn();
      const stop = startToolHeartbeat({ writer: { write } }, {
        tool: "t",
        intervalMs: 100,
      ...
      expect(write).not.toHaveBeenCalled();
      vi.advanceTimersByTime(1); // 满 1000
      expect(write).toHaveBeenCalledTimes(1);
      stop();
    });

    it("过大(999999)被夹到上界 30000:30000ms 才 emit 第一次", () => {
      const write = vi.fn();
      const stop = startToolHeartbeat({ writer: { write } }, {
        tool: "t",
        intervalMs: 999_999,
      ...
      expect(write).not.toHaveBeenCalled();
      vi.advanceTimersByTime(1); // 满 30000
      expect(write).toHaveBeenCalledTimes(1);
      stop();
    });
  });
  ...
  it("write 同步抛错被吞掉,后续 tick 仍照常 emit", () => {
    ...
    expect(() => vi.advanceTimersByTime(1_000 * 3)).not.toThrow();
    expect(write).toHaveBeenCalledTimes(3); // 吞错后定时器存活、继续 emit

    stop();
  });

  it("write 时而抛错时而成功:抛错的 tick 不阻断后续成功 tick(seq 仍持续推进)", () => {
