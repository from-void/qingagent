...
import { branchCall, getSessionSnapshot } from "../llm/modelConfig.js";
...
只输出标题本身，不要引号、书名号、Markdown 标记、解释或句号；不超过 ${MAX_TITLE_CHARS} 个字符。`,
  ...
  if (abortSignal?.aborted) return null;
  state.branchTitleGenerated = true;
  if (!result.ok) return fallbackTitle;
  return normalizeGeneratedTitle(result.text) ?? fallbackTitle;
}
