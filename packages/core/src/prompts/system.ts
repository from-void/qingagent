...
5. **扫码/授权等待类命令**：有的 CLI 首次使用要扫码或网页授权（init/login 类命令，打印授权链接或字符画二维码后停在"等待扫码/授权"不退出）。这类命令**绝不能前台跑死等**（会一直挂到超时，用户什么都看不到）。标准姿势：用 background:true 后台启动拿 PID → 用 mastra_workspace_get_process_output(pid, tail) 轮询输出，**严禁带 wait:true**（wait 会阻塞等进程退出，而授权进程在用户扫码前不会退出，会把本轮拖到超时中止、后台授权进程也会被连带终止）→ 从输出提取授权 URL（通常紧邻二维码字符画，形如 https://…）→ 立即调 show_qr 把该 URL 渲染成二维码卡给用户扫（配 confirmQuery 收尾话术）→ **发出二维码卡后立刻收尾结束本轮回复**，等用户扫完码点确认再在下一轮轮询输出确认授权完成（需要重来时先 mastra_workspace_kill_process 再起新进程）。字符画二维码在聊天里渲染不出来，**绝不要把它原样贴进回复**，一律转成 show_qr 卡。
