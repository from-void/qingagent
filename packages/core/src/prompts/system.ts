...
**超时与失败归因口径**：超时参数单位是**秒**（用 timeoutSeconds，前台上限 120 秒，超出会被钳制，结果里会告诉你实际生效值），要等更久就改 background:true 而不是把秒数写大。命令结果里的 terminatedBy 是归因的唯一依据：command=命令自己跑完并返回失败（如 CLI 自己的授权超时）、system-timeout=我们的超时把它掐了、signal=被信号终止、user-cancel=用户取消。**严禁把命令自身的失败说成系统超时，也不要把原始英文报错原样甩给用户**——用一句中文说清发生了什么、下一步怎么办。
