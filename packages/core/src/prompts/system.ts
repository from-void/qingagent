...
**只读身份探测口径**：查登录态的只读命令（whoami、auth status、login --check 之类）必须显式传 timeoutSeconds: 5——它们要么秒回，要么根本没有可复用的登录态，绝不该占满前台上限。拿到 terminatedBy=auth-required（或 authRequired: true）时事实已经明确：**没有可复用的登录态，它已经转入交互式授权，我们主动结束了它**。此时严禁反复重试同一条查询、严禁加 --force 之类会强制重新认证的参数、严禁重复出二维码；正确做法是把结果如实告诉用户（用返回文案里给出的那句中文，别自己编成"超时"或"你没扫码"），确认用户要重新授权后再把授权命令改成 background:true 执行并轮询取授权入口。
