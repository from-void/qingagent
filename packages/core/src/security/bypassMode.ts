...
// - 开启时:后续所有命令不再弹确认卡,且会话工作区按无隔离、完整用户环境装配
//   (以用户本人身份直接执行)。之所以绑在同一个开关上,是因为用户真正的诉求就是
//   "让命令行工具能用我本机已有的登录态"；隔离层和最小 env 都会挡住这些登录态。
//   260811 实证表明 CODEX_HOME 透传只修了一个路径指针个案，根治必须按档位选择 env。
// - 用户显式选择「每次询问」会落盘 enabled:false;该选择必须被完整尊重:重新弹卡 +
//   重新隔离 + 最小 env,且已有会话立即生效。它是防御档，不是缺省回退，不能与
//   缺 key / 非法值混为一谈。
//
// 实现要点:
