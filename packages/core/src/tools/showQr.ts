...
export const showQrTool = createTool({
  ...
  description:
    ...
    "卡片会自动在过期后作废并给出刷新入口。" +
    "【禁止重复出码】只要 CLI 输出里已经出现「扫码成功 / 已拿到凭据 / token obtained」这类信号," +
    "授权环节就已经完成:此后**禁止**再调用本工具出新码,也**禁止**重跑会重新出码的登录命令," +
    "更不许对用户说「你没有完成扫码」。这时的失败在保存登录信息那一步,重复出码只会让用户白等一轮又一轮。" +
    "遇到这种情况立即停下,如实说明是保存没成功,不要靠反复重试碰运气。" +
