...
export const githubListReposTool = createTool({
  ...
  description: "列出 GitHub 仓库。已连接时 owner 可省略并列出当前用户可见仓；匿名模式必须提供 owner，只列公开仓。",
  inputSchema: z.object({ owner: z.string().min(1).max(100).optional() }),
  execute: async (input, context) => {
    ...
    try {
      ...
      const response = await client.listRepos(input.owner, context?.abortSignal);
      ...
      return { repos, count: repos.length, anonymous: !connected, rateLimit: response.rateLimit };
    } finally { stop(); }
  },
});
