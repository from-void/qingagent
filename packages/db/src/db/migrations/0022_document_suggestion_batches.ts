...
async function up(client: Parameters<Migration["up"]>[0]): Promise<void> {
  const statements = [
    `CREATE TABLE document_suggestions_new (
      ...
      PRIMARY KEY (doc_id, base_version, batch_id, id)
    )`,
    `INSERT INTO document_suggestions_new (
      ...
      id, doc_id, base_version, 'legacy', status, anchor_json, steps_json,
      preview_json, summary, conflict_json, kind, note, origin, group_id,
      group_meta_json, created_at, updated_at, severity
    ...
    "CREATE INDEX idx_document_suggestions_batch ON document_suggestions(doc_id, base_version, batch_id, status)",
    "ALTER TABLE document_drafts ADD COLUMN batch_id TEXT NOT NULL DEFAULT 'legacy'",
  ];
  for (const sql of statements) await client.execute(sql);
}

export const migration0022DocumentSuggestionBatches: Migration = {
  id: 22,
  name: "document_suggestion_batches",
  up,
};
