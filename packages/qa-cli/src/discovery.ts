...
export async function discoverInstance(filePath = instanceFilePath()): Promise<InstanceInfo> {
  ...
  try {
    ...
    throw new QaCliError("NO_INSTANCE", "请先打开清简应用");
  }
  if (!isPidAlive(info.pid)) throw new QaCliError("NO_INSTANCE", "请先打开清简应用");
  const res = await fetch(`http://127.0.0.1:${info.port}/api/v1/external/health`, {
    headers: { Authorization: `Bearer ${info.token}` },
  }).catch(() => null);
  if (!res) throw new QaCliError("NO_INSTANCE", "请先打开清简应用");
  if (res.status === 401) throw new QaCliError("AUTH_FAILED", "实例可能已重启");
  if (!res.ok) throw new QaCliError("NO_INSTANCE", "请先打开清简应用");
  return info;
}
