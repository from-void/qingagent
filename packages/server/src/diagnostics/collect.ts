...
export async function collectFrameLogs(
  ...
  const { sessionManager } = await import("../bridge/bridgeHandler.js");
  ...
  for (const sessionId of sessionIds) {
    try {
      ...
      if (read.frames.length === 0) continue;
      const lines = read.frames.map((entry) => (
        JSON.stringify(redactFrameEntry(entry, privacyLevel))
      ));
      out.push({
        ...
        mtime: lastFrameTime(read.frames),
      });
    } catch {
