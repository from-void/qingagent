...
dataAdminRoutes.get("/data/usage/export", async (c) => {
  ...
  try {
    result = await client.execute("SELECT created_at, session_id, call_site, model_id, key_origin, input_tokens, output_tokens, cache_hit_tokens, cache_miss_tokens FROM llm_usage_events ORDER BY created_at");
  } catch (err) {
    if (!isMissingUsageTableError(err)) throw err;
  }
  const header = "created_at,session_id,call_site,model_id,key_origin,input_tokens,output_tokens,cache_hit_tokens,cache_miss_tokens";
  const lines = (result?.rows ?? []).map((r) => {
    const row = r as unknown as Record<string, unknown>;
    // CSV 注入防护:统一包引号、转义内部引号,并中和电子表格公式前缀。
    return [
      row.created_at, row.session_id, row.call_site, row.model_id, row.key_origin,
